...
\paragraphe{Tu peux soustraire des chemins entiers à la lecture de l'agent. Un
fichier d'ignore dédié dit à Claude Code de ne pas ouvrir certains dossiers,
même s'ils sont versionnés :}

\begin{extraitcode}{text}
# .claudeignore — fichiers que l'agent n'ouvrira pas
.env
.env.*
secrets/
**/*.pem
...
\resumeChapitre{
  ...
  \element{\important{Secrets} : les clés et identifiants vont dans \code{.env} ignoré par Git ; utilise \code{.claudeignore} pour soustraire les fichiers sensibles à la lecture de l'agent.}
