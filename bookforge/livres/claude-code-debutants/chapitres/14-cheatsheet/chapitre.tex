...
/usage         # Afficher l'usage et les limites disponibles
...
\begin{extraitcode}{bash}
# Voir ou ajuster les permissions pendant la session
/permissions

# Lancer en mode plan : explorer avant de modifier
claude --permission-mode plan

# Lancer en mode acceptEdits : autoriser les éditions dans le dépôt
claude --permission-mode acceptEdits

# Lancer en mode autonome (approuve tout automatiquement)
# À réserver aux environnements isolés
...
\paragraphe{Les scripts appelés par ces hooks lisent le contexte de l'action en
JSON sur \code{stdin}. Évite les exemples qui supposent des variables
d'environnement magiques : elles vieillissent mal et cassent en silence.}
...
claude mcp add --transport http mon-api --scope project https://mcp.example.com/mcp
\end{extraitcode}

\section{Surveillance des coûts}

\begin{extraitcode}{bash}
# Usage et limites disponibles (dans la session)
/usage

# Voir l'usage côté Console Anthropic
...
\resumeChapitre{
  ...
  \element{\important{Coûts} : \code{/usage} en session, sessions courtes et
