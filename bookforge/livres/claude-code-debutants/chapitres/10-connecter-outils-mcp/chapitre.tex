...
\paragraphe{La configuration tient dans un fichier. Tu déclares chaque serveur :
un nom, la commande qui le lance, et ses éventuels paramètres. Voici une
intégration de recherche documentaire, branchée sur tes pages internes.}
...
\begin{extraitcode}{json}
{
  "mcpServers": {
    "docs-interne": {
      "command": "npx",
      "args": ["-y", "@exemple/mcp-docs"],
      "env": {
        "DOCS_URL": "https://wiki.interne.exemple",
...
\paragraphe{Note le \code{$DOCS_TOKEN} : le secret ne vit pas en clair dans le
fichier, il vient d'une variable d'environnement. On verra plus loin pourquoi ce
réflexe compte. Une fois le fichier en place, tu vérifies que le serveur est bien
vu par l'agent.}
...
# Demarrer une session et inspecter les outils charges
claude
> /mcp
\end{extraitcode}
